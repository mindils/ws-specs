...
- [ ] Сервис проверки и тексты
- [ ] Запись результата по типам, `TR1`
- [ ] Хук в диспетчере
- [ ] Расширение `pack_checked_text`
- [ ] Тесты
- [ ] Передать результат на независимую проверку.

Ближайший шаг: прочитать `PacketParserService.dispatchAndResolve` и `DiadocPacketCheckResultService`, спроектировать `appendResults`.
Препятствия: нет
