...
- [ ] Модель, changelog, DTO, бандл
- [ ] Алгоритм сборки и запись
- [ ] `test-026` и тесты
- [ ] Передать результат на независимую проверку.

Ближайший шаг: подключить `test-026`, прогнать существующие SCHF-тесты как базу, затем менять `PacketAssemblyService`.
Препятствия: нет
