...
- [x] Java-тесты: снять квалификатор, поправить комментарии (25 файлов —
      перечисленные в «Области» плюс `TechClaimDemoDataIT`,
      `ContractSupplyOutLookupIT`, `RepairNoticeSendingServiceIT`, где были
      те же вхождения)
- [x] Тестовые `*.sql` Liquibase (7 файлов)
- [x] Правило в javadoc `BaseIT`
- [x] Самопроверка и полный прогон: схема пересоздана, `./gradlew :app:test`
      — 1536 passing, 12 pending, 0 failing; `spotlessCheckAll` — успешно
- [x] Передать результат на независимую проверку.
- [x] Независимая проверка 1: все критерии подтверждены, итог `pass` —
      [checks/001.md](checks/001.md).

Ближайший шаг: таск закрыт; снят блокер [T06](../T06/task.md) — его
критерий C4 переподтверждается повторной независимой проверкой.
Препятствия: нет
