...
- [x] 13 XML переведены на `rvkFilter`.
...
Ближайший шаг: выполнение или проверка таска [T02](../../tasks/T02/task.md).
Препятствия: нет

## Пересмотр 2026-09-17

По [Q01](../../questions/Q01.md) поле `defectReason.defectReasonName` в
`cnt-defect-list-view.xml` заменяется entity-полем `defectReason`, ключ
`cntDefectListView.filter.defectReason` удаляется — [T05](../T05/task.md).
Остальные 12 справочников не затронуты; статус T03 остаётся `done`.
