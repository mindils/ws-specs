...
Статус: done — проверка 001 пройдена
...
- [x] Реестр, ремонты, освидетельствования.
- [x] Акты, договоры, гарантии.
- [x] Ключи messages согласованы.
- [x] `ContainerRvkFilterUiTest` и открытие актов/гарантий тестом.
- [x] Передать результат на независимую проверку.
- [x] Независимая проверка пройдена: [checks/001.md](checks/001.md) — `pass`.

Реализация завершена, отчёт — [result.md](result.md), итерация 1. Самопроверка
зелёная: `spotlessApply :app:compileJava :app:compileTestJava`,
`:app:test --tests 'ru.fgk.ws.app.container.view.*'` (26 passing),
`spotlessCheckAll`. Запасной `jpqlFilter` для `container.contNum` не
понадобился — ловушка префикса `container_` на `rvkFilter` не воспроизводится.

Проверка 001 (2026-09-16) подтвердила C1–C4, негативные сценарии и регрессии:
`:app:test --tests 'ru.fgk.ws.app.container.*'` — 76 passing,
`spotlessCheckAll --rerun-tasks` — BUILD SUCCESSFUL, браузерный проход по шести
страницам под `user_read`/`user_edit` (пометка `render not browser-verified`
снята).

Ближайший шаг: таск T04 — сквозная проверка раздела.
Препятствия: нет

## Пересмотр 2026-09-17

По [Q01](../../questions/Q01.md) выпадающие списки по справочникам
переносятся внутрь `rvkFilter` как entity-поля, а текстовые поля по пути
через ссылку на `VOrgPassport`, `ContainerContract`, `cnt_*` становятся
выбором сущности — [T05](../T05/task.md). Статус T02 остаётся `done`, но
проверка 001 в части C1 (выпадающие списки над `rvkFilter`) и теста
пересечения со штатным фильтром устарела; актуальное состояние страниц
подтверждает проверка T05.
