...
- [x] Правка `IframeSecurity`, комментарий.
- [x] Самопроверка `curl`.
- [x] Передать результат на независимую проверку.
- [x] Независимая проверка [checks/001.md](checks/001.md) пройдена.

Ближайший шаг: выполнение следующего таска волны пересмотра UI по plan.md — [T11](../T11/task.md).
Препятствия: нет
