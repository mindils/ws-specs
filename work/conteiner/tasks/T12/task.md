...
- [ ] XML: tabSheet, пять вкладок, распределение полей.
- [ ] Контроллер: просмотр только при файле, переключение вкладки при
      ошибке валидации.
- [ ] Messages, UI-тест, contracts.md.
- [ ] Передать результат на независимую проверку.

Ближайший шаг: открыть `container-repair-detail-view.xml` и таблицу
распределения полей в contracts.md.
Препятствия: нет
