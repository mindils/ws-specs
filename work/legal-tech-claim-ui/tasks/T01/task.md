...
Не трогать: `nsi/**`, `legal/caseone/**`, `sso-plagin`.
...
- [ ] Переименовать каталоги, файлы и классы (`git mv`), обновить ссылки.
- [ ] Changelog-и таблиц и view, константы БД.
- [ ] i18n-ключи и тесты.
- [ ] Документ отличий.
- [ ] Пересоздать БД, пройти самопроверку, записать `result.md`.
- [ ] Передать результат на независимую проверку.

Ближайший шаг: `git status --short app/src/main/java/ru/fgk/ws/app/legal`
и `ls app/src/main/java/ru/fgk/ws/app/legal/warrantyrepair/*` — составить
список файлов к переименованию.
Препятствия: нет.
