...
- [ ] Выписать формулы факта завершения и недостающих колонок из SQL.
- [ ] Второй changeSet view: join расчёта, сроки, степени,
      `deadlines_outdated`, колонки отчёта.
- [ ] Перегенерировать `VLegalTechClaim` и i18n из структуры view.
- [ ] Дополнить `TechClaimViewIT` (степени, актуальность, фильтр в БД).
- [ ] Самопроверка, `result.md`.
- [ ] Передать результат на независимую проверку.

Ближайший шаг: открыть `legal_v_tech_claim.xml` и `LegalTechClaimDeadline`,
сопоставить имена 28 колонок дат с семью этапами.
Препятствия: нет.
