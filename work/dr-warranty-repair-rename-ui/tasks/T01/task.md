...
Ветка: `git checkout f/caseone-task` (или новая от неё).
...
Гейт 2: `./gradlew :app:test` — контекст поднимается.
...
- [ ] Переключиться на `f/caseone-task`, поднять Docker, убедиться, что БД проекта доступна.
- [ ] Переименовать колонки и файлы в четырёх changelog-ах; индекс отцепки сделать уникальным.
- [ ] Переименовать поля и аксессоры в четырёх entity.
- [ ] Переименовать i18n-ключи; проверить отсутствие осиротевших.
- [ ] Пересоздать локальную БД, прогнать `compileJava`, `spotlessCheckAll`, `:app:test`.
- [ ] Сверить entity ↔ changelog ↔ БД по C3–C5.
- [ ] Передать результат на независимую проверку.

Ближайший шаг: `git checkout f/caseone-task` и сверить фактический список
колонок в четырёх changelog-ах с таблицами переименований выше.
Препятствия: нет.
